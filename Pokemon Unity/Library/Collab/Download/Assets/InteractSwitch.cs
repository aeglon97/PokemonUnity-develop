...
public class InteractSwitch1 : MonoBehaviour
{
    ...
    public string[] spanish = { "videojuegos" };
    public string[] english = { "video games" };
    ...
    private IEnumerator runEvent(CustomEventTree[] treesArray, int index)
    {
        ...
        switch (ty)
        {
            ...
            case (CustomEventDetails.CustomEventType.Dialog):
                ...
                {
                    print(currentEvent.bool0);
                    if (currentEvent.bool0 && i > 0)
                    {
                        yield return Dialog.StartCoroutine("scrollText", currentEvent.float0);
                        yield return StartCoroutine(Dialog.drawTextSilent(english[i]));
                    }
                    else
                    {
                        Dialog.drawDialogBox();
                        GlobalVariables.global.debug(name);
                        //if object is Switch
                        yield return StartCoroutine(Dialog.drawText(english[i]));
                    }
